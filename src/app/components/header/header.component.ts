...
export class HeaderComponent {
  ...
  ngOnInit() {
    const storedLikes =
      this.localStorageService.getItem<{ id: number; time: number }[]>('likes');
  ...
  open() {
    ...
    if (
      !this.arraysAreEqual(
        this.likesList,
        this.localStorageService.getItem<{ id: number; time: number }[]>(
          'likes',
        ) ?? [],
      )
    ) {
  ...
  arraysAreEqual(
    arr1: { id: number; time: number }[],
    arr2: { id: number; time: number }[],
  ) {
